...
while True:
    ...
    for event, _ in events:
        ...
        if event_socket == server_socket:   # Если событие произошло с серверным сокетом, была попытка подключения
            connection, address = server_socket.accept()
            connection.setblocking(False)
            print(f"Получен запрос на подключение от {address}")
            selector.register(connection, selectors.EVENT_READ)     # Зарегистрировать клиент, подключившийся к сокету
        else:
            data = event_socket.recv(1024)  # Если событие произошло не с серверным сокетом, получить данные от клиента
