...
class ConnectedSocket:
    ...
    async def __aenter__(self):     # Эта сопрограмма вызывается при входе в блок with.
                                    # Она ждет подключения клиента и возвращает это подключение.
    ...
    async def __aexit__(self,
                        ...
                        exc_tb: Optional[TracebackType]):   # Эта сопрограмма вызывается при выходе из блока with.
...
async def main():
    ...
    async with ConnectedSocket(server_socket) as connection:    # Здесь вызывается __aenter__ и ожидаем подключения
